...
def compute_flow(rho,psi):
	...
	'''
	flow = torch.zeros_like(rho)
	logmean,dmean_d1 = logarithmic_mean(rho)
	gradpsi = grad_psi(psi)
	stationary_pi_reshape = torch.tile(stationary_pi,(1,3)).reshape(1,3,3) # bz x 3 x 3
	flow = gradpsi*transition_matrix*stationary_pi_reshape # bz x 3 x 3
	return flow
...
def compute_distance(rho,psi):
	...
	logmean,dmean_d1 = logarithmic_mean(rho)
	gradpsi = grad_psi(psi)
	stationary_pi_reshape = torch.tile(stationary_pi,(1,ndim)).reshape(1,ndim,ndim) # bz x 3 x 3
	distance = torch.sqrt(torch.mean(torch.sum((gradpsi**2)*logmean*transition_matrix*stationary_pi_reshape,dim=(1, 2))/2)) # bz x 3 x 3
	return distance
...
def compute_flow(rho,psi):
	...
	return flow
def pde_baseline(t,net_rho,net_psi,loss):
	rho = net_rho(t)
	psi = net_psi(t)
	bz = t.shape[0]
	all_zero = torch.zeros((bz,ndim)).to(device)
	...
	# Use Energy instead of Geodesics equation
	# Geodesics equation
	# http://etheses.dur.ac.uk/14124/1/Kamtue_thesis.pdf?DDD21+ 
	# page 161 Eq. 21.3
	stationary_pi_reshape = torch.tile(stationary_pi,(1,3)).reshape(1,3,3) # bz x 3 x 3
	pde2 = torch.mean(torch.sum((gradpsi**2)*logmean*transition_matrix*stationary_pi_reshape,dim=(1, 2))/2) # bz x 3 x 3
	
	return pde1,pde2
# def energy_line(x1, x2, num_points=100):
# 	"""
# 	Generate a linearly spaced set of points between two points in 3D space.
	
# 	Parameters:
# 	- x1: The first point (numpy array or list).
# 	- x2: The second point (numpy array or list).
# 	- num_points: The number of points to generate.
	
# 	Returns:
# 	- A numpy array of shape (num_points, 3) containing the points on the line.
# 	"""
# 	rho = torch.from_numpy(np.linspace(x1, x2, num_points)).float().to(device) # bz x 3
# 	logmean,_ = logarithmic_mean(rho) # bz x 3 x 3
# 	gradpsi = theta_gradpsi/logmean
# 	stationary_pi_reshape = torch.tile(stationary_pi,(1,3)).reshape(1,3,3) # bz x 3 x 3
