...
from nn_planar import *
from ot_planar import *
...
if __name__ == "__main__":
    ...
    if args.model == "poly":
         net_rho = polyNN(1,ndim,activation_f,projection_p).to(device)
    if args.model == "basic":
         net_rho = basicNN(1,ndim,activation_f,projection_p).to(device)
    if args.model == "barycenter":
         net_rho = barycenterNN(1,ndim,activation_f).to(device)
    if args.model == "barycenteradd":
         net_rho = barycenteraddNN(1,ndim,activation_fn=activation_f,transform_fn=None,invert_coeff=start_invert_coeff).to(device)
